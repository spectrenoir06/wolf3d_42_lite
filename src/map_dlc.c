...
/*   Updated: 2016/03/15 19:48:22 by spectre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		map_load_data(t_map *map, char *path, t_mapload *ml)
{
	...
	{
		ml->nb = ft_itoa(ml->i);
		ft_kebab(ml->buff, path, "textures/", ml->nb, ".png", NULL);
		tmp = IMG_Load(ml->buff);
		map->textures[ml->i++] = SDL_ConvertSurface( tmp, map->bmpFormat->format, 0 );
		SDL_FreeSurface(tmp);
		free(ml->nb);
	}
	colors_init(map);
	ft_kebab(ml->buff, path, "textures/sky.png", NULL);

	tmp = IMG_Load(ml->buff);
	map->sky = SDL_ConvertSurface( tmp, map->bmpFormat->format, 0 );
	SDL_FreeSurface(tmp);
